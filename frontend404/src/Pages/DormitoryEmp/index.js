...
const DormitoryEmp = (props) => {
  ...
  const [data, setData] = React.useState([ 
    ...
    {'building': 'centeral', 'room': '36', 'current_capacity': 3, 'users': [{'token': token},{'token': 'fjsdkljfksdjfldsjlfjsdjlj'}] }, 
  ...
  const handleYesDialog =() =>{
    ...
    axios({
      method: "post",
      url: "http://404g.pythonanywhere.com//dormitory/reserveDormitorie",
      headers: {
  ...
  const fillRows = () =>{
    ...
    data.map((item, index) => {
      ...
      item.users.map((user)=> {if( user.token === token) {
  ...
  const initialize = () => {
    ...
    axios({
      method: "get",
      url: "http://404g.pythonanywhere.com//dormitory/getDormitories",
      headers: {
        "Content-Type": "application/json",
        Authorization:
        "Bearer " + token,  
          ...
          >
            <AppBar color="primary" position="static"     style={{ height: 64 }}>
              <Toolbar style={{ height: 64, justifyContent: "space-between", }}>
                <Typography color="inherit">Dormitory {'Employee'}</Typography>
              </Toolbar>
            </AppBar>
            {isLoading ? (
